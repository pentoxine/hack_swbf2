...
namespace menu {
	...
	void init() {
		...
		if (isOpen)
		{
			ImGui::Begin("HACK_SWBF2 by pentoxine", &isOpen, window_flags);
			if (ImGui::TreeNode("ESP")) {
				ImGui::Checkbox("Enable ESP", &settings::ESP::enabled);
				ImGui::Checkbox("Show ennemies", &settings::ESP::ennemies);

				ImGui::TreePop();
			}
			ImGui::Text("Press INSERT to open/close menu.");
			ImGui::Text("GitHub repo: https://github.com/pentoxine/hack_swbf2");
			ImGui::End();
		}
	}
}